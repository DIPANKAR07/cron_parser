...
const FieldParser_1 = require("../src/parser/FieldParser");
describe("FieldParser", () => {
    test("handles wildcard", () => {
        const parser = new FieldParser_1.FieldParser(0, 5);
        expect(parser.parse("*")).toEqual([0, 1, 2, 3, 4, 5]);
    });
    test("handles step values", () => {
        const parser = new FieldParser_1.FieldParser(0, 59);
        expect(parser.parse("*/15")).toEqual([0, 15, 30, 45]);
    });
    test("handles ranges", () => {
        const parser = new FieldParser_1.FieldParser(1, 10);
